...
int main() {
	// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-= TELEGRAM BOT =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	Bot bot("6271070372:AAGQyig0xAJcBNmkKLFpQ2GCdLP_iEGlxHA"); // bot token
	...
	{
    int stopCicle = 0;
		if (!newEn.empty() && !newTrans.empty() && indxCount > 1)
		{
			do {
                ++stopCicle;
			...
			while (lastIndx == randIndx && stopCicle <= 3);
			lastIndx = randIndx;
	...
	{
		randIndx = 0;
		if (!newEn.empty() && !newTrans.empty() && indxCount > lastWords)
		{
          int stopCicle = 0;
            if (!newEn.empty() && !newTrans.empty() && indxCount > 1)
		    {
